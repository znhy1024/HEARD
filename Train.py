...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model,rd_lr):
        ...
        if self.best_score is None:
            ...
            if rd_lr < self.stop_rate or self.count>self.patience:
                ...
                print('Least Validation loss ({:.6f}).'.format(-self.val_loss_min))
        else:
            self.best_score = score
            self.save_checkpoint(self.best_score, model)
            self.count = 0
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        torch.save(model.state_dict(),self.model_dir['loss']+'best_model'+'.m')
...
class Train(object):
    ...
    def __init__(self,config):
        ...
        self.model_dir = config["models"][config["active_model"]]["model_dir"]+config["active_model"]+'/'

        self.decay_patience = config["models"][config["active_model"]]["hyperparameters"]["decay_patience"]
        self.stop_rate = config["models"][config["active_model"]]["early_stop_lr"]
        self.stop_patience = config["models"][config["active_model"]]["early_stop_patience"]
        
        self.interval = config["models"][config["active_model"]]["hyperparameters"]["interval"]

        self.val_len,self.val_loader,self.folds_loader = get_dataloader(config)
               
    
    def set_optimizer(self,config,model):
        optimizer = torch.optim.Adam([
            {'params':model.parameters(),'lr':config["models"][config["active_model"]]["hyperparameters"]["learning_rate"]['RD']}
    ...
    def train_HEARD(self,fold,train_loader,val_loader):
        ...
        n_epochs = self.config["models"][self.config["active_model"]]["hyperparameters"]["epochs"]
        ...
        early_stopping = EarlyStopping(self.stop_rate,self.stop_patience,model_dir)
        for epoch in range(n_epochs):
            ...
            print(f'start training {epoch} epoch: '+time.strftime("%Y-%m-%d %H:%M:%S",time.localtime()))

            for Batch_data in train_loader:
               
                label_seqs,post_indexes,real_lens = Batch_data[0],Batch_data[3],Batch_data[10]
                outputs = model(Batch_data)
                loss,stop_points,stop_preds,_ = model.compute_log_likelihood(Batch_data)
                ...
                stops_index = post_indexes.cpu().data[torch.arange(0,len(stop_preds)),stop_points]*1.0
            ...
            print("Epoch {:05d} | Train_Loss {:.4f}| Train_Acc {:.4f} | ER : {:.4f}| Train_R {:.4f} | Train_P {:.4f} | Train_F {:.4f} | lr : {}|".format(epoch,
                                                                                                    np.mean(avg_loss),train_acc,torch.mean(early_rate).cpu().data,train_recall,train_precision,train_f1,
                                                                                                    lr_n))

            print(f'[+]Start eval {epoch} epoch: '+time.strftime("%Y-%m-%d %H:%M:%S",time.localtime()))
            ...
            with torch.no_grad():
                ...
                for Batch_data in val_loader:
                    ...
                    loss,stop_points,stop_preds,_ = model.compute_log_likelihood(Batch_data,if_dp=False)
    ...
    def test_HEARD(self,test_loader,model_paras):
        ...
        with torch.no_grad():
            ...
            for Batch_data in test_loader:
                label_seqs,seq_lengths,post_indexes,real_lens,eids,tids = Batch_data[0],Batch_data[6],Batch_data[3],Batch_data[10],Batch_data[11],Batch_data[12]

                outputs = model(Batch_data,if_dp=False)

                eid = eids[0]
                timestamp = Batch_data[7][0].cpu().data.tolist()
                predicts[eid] = {'preds':outputs[0][2][:,0].cpu().data.tolist(),'time':timestamp,'label':label_seqs.cpu().data.tolist()[0]}

                _,stop_points,stop_preds,delta_N_at_stops = model.compute_log_likelihood(Batch_data,if_dp=False)
